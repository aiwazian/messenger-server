	...
	imports: [
		...
		ConfigModule.forRoot({
			...
			validationSchema: Joi.object({
				...
				/*
				 * Публичный бакет и домен его раздачи.
				 *
				 * Стикеры отдаются без подписи, поэтому лежат в отдельном бакете:
				 * у провайдера публичный доступ включается на бакет целиком, а не
				 * на отдельный объект или префикс.
				 *
				 * CDN_PUBLIC_BASE_URL — домен перед этим бакетом. Отдаётся клиенту
